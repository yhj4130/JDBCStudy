...
SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC = '';
...
SELECT * FROM (SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID JIKWI_ID, BASICPAY, SUDANG FROM TBL_EMP ORDER BY BASICPAY DESC) WHERE EMP_ID = 1001;

SELECT *
FROM(
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_LOC, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG
...
--7. 사번	이름	주민번호	입사일	지역	전화번호	부서	직위	기본급	수당	급여
...
WHERE E.CITY_ID = C.CITY_ID
  AND E.BUSEO_ID = B.BUSEO_ID
  AND E.JIKWI_ID = J.JIKWI_ID;
--ORDER BY E.EMP_ID         -- 사번 정렬
--ORDER BY E.EMP_NAME       -- 이름 정렬
--ORDER BY B.BUSEO_NAME     -- 부서 정렬
--ORDER BY J.JIKWI_NAME     -- 직위 정렬
--ORDER BY PAY DESC         -- 급여 내림차순 정렬


--8. 직원 데이터 전체 조회 쿼리문 (사번/이름/부서/직위/급여 내림차순)
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY
FROM EMPVIEW
ORDER BY EMP_ID;
--> 한 줄 구성
...
--9. 직원 검색 쿼리문 구성 (사번/이름/부서/직원)
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY
FROM EMPVIEW
WHERE EMP_NAME='홍길동';
--> 한 줄 구성
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW WHERE EMP_NAME='홍길동'
;


--10. 직원 데이터 수정 쿼리문 구성
UPDATE TBL_EMP
SET EMP_NAME='이원일', SSN='801212-1234567', IBSADATE=TO_DATE('2012-11-11', 'YYYY-MM-DD')
...
1062	이원영	901212-1234567	2010-11-11	서울	010-1234-1234	개발부	대리	1250000	2160000	2000000	4160000
1061	윤희진	96100-0000000	2019-04-04	제주	010-0040-0404	홍보부	차장	2350000	60000000	1000000	61000000
*/
