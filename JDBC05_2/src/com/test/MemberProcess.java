...
public class MemberProcess
{
	...
	public void memberInsert()
	{
		...
		try
		{
			...
			// 부서 리스트 구성
			ArrayList<String> buseo = dao.selectBuseo();
			...
			// 직위 리스트 구성
			ArrayList<String> jikwi = dao.selectJikwi();
	...
	public void memberUpdate()
	{
		...
		try
		{
			 ...
			 * MemberDTO dto = new MemberDTO();
			 * 
			 * for (MemberDTO memberDTO : members) { dto = memberDTO; }
			 * 
			 * // 지역 리스트 구성 ArrayList<String> citys = dao.selectCity(); StringBuilder
			 * cityStr = new StringBuilder(); for (String city : citys) cityStr.append(city
			 * + "/");
			 * 
			 * // 부서 리스트 구성 ArrayList<String> buseos = dao.selectBuseo(); StringBuilder
			 * buseoStr = new StringBuilder(); for (String buseo : buseos)
			 * buseoStr.append(buseo + "/");
			 * 
			 * // 직위 리스트 구성 ArrayList<String> jikwis = dao.selectJikwi(); StringBuilder
			 * jikwiStr = new StringBuilder(); for (String jikwi : jikwis)
			 * jikwiStr.append(jikwi + "/");
			 * 
			 * 
			 * 
			 * // 사용자에게 보여지는 화면 처리 System.out.println(); System.out.
			 * println("직원 수정 정보 입력 ---------------------------------------------------------"
			 * ); System.out.printf("기존 이름 : %s\n", dto.getEmpname());
			 * System.out.print("수정 이름 : "); String empName = sc.next();
			 * if(!empName.equals("-")) dto.setEmpname(empName);
			 * 
			 * System.out.printf("기존 주민등록번호(yymmdd-nnnnnnn) : %s\n", dto.getSsn());
			 * System.out.print("수정 주민번호(yymmdd-nnnnnnn) : "); String ssn = sc.next();
			 * if(!ssn.equals("-")) dto.setSsn(ssn);
			 * 
			 * System.out.printf("기존 입사일(yyyy-mm-dd) : %s\n", dto.getIbsaDate());
			 * System.out.print("수정 입사일(yyyy-mm-dd) : "); String ibsaDate = sc.next();
			 * if(!ibsaDate.equals("-")) dto.setIbsaDate(ibsaDate);
			 * 
			 * System.out.printf("기존 지역 : %s\n", dto.getCityLoc());
			 * System.out.printf("수정 지역(%s) : ", cityStr.toString()); String cityLoc =
			 * sc.next(); if(!cityLoc.equals("-")) dto.setCityLoc(cityLoc);
			 * 
			 * System.out.printf("기존 전화번호 : %s\n", dto.getTel());
			 * System.out.print("수정 전화번호 : "); String tel = sc.next(); if(!tel.equals("-"))
			 * dto.setTel(tel);
			 * 
			 * System.out.printf("기존 부서 : %s\n", dto.getBuseoName());
			 * System.out.printf("수정 부서(%s) : ", buseoStr.toString()); String buseoName =
			 * sc.next(); if(!buseoName.equals("-")) dto.setBuseoName(buseoName);
			 * 
			 * System.out.printf("기존 직위 : %s\n", dto.getJikwiName());
			 * System.out.printf("수정 직위(%s) : ", jikwiStr.toString()); String jikwiName =
			 * sc.next(); if(!jikwiName.equals("-")) dto.setJikwiName(jikwiName);
			 * 
			 * System.out.printf("기존 기본급 : %s\n", dto.getBasicPay());
			 * System.out.printf("수정 기본급(최소 %d 이상) : ",
			 * dao.searchBasicPay(dto.getJikwiName())); String basicPay = sc.next();
			 * if(!basicPay.equals("-")) dto.setBasicPay(Integer.parseInt(basicPay));
			 * 
			 * System.out.printf("기존 수당 : %d\n", dto.getSudang());
			 * System.out.print("수정 수당 : "); String sudang = sc.next();
			 * if(!sudang.equals("-")) dto.setSudang(Integer.parseInt(sudang));
			 * 
			 * System.out.println();
			 */
			...
			if (members.size() > 0)
			{
				...
				System.out.printf("수정 직위(%s) : ", cityStr);
				String cityLoc = sc.next();
				if (cityLoc.equals("-"))
					cityLoc = mCityLoc;
